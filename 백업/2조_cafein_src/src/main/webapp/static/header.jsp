...
function onMessage(evt) { // 메시지 받으면 
	...
	var text = evt.data; // 내가 입력한 text    
    var array = new Array();
	array = text.split('&');
	var realText = array[0];  // 입력 텍스트
	var realUserid = array[1]; // 보낸 사람 id
                            ...
                            <div class="classynav">
								...
								<c:if test="${not (empty sessionScope.userid) && (sessionScope.userid ne 'wlsdnd123')}"> <!-- 세션이 있어야 마이페이지 보이게 -->
                                    ...
                                    <c:if test="${not (empty sessionScope.userid) && (sessionScope.userid ne 'wlsdnd123')}"> <!-- 세션이 있으면 메시지함 보이게 -->                       
										<div class="cart-btn">
	                                    <div id="toggle"><p><a href="javascript:void(window.open('/coffee/chatRoom', '채팅방','width=370, height=550 top=180, left=1450'))">
	                                    <span class="glyphicon glyphicon-envelope" style="color:#FFFFFF;"></span> 
	                                    <span class="quantity" id="spanResult" ></span></a></p></div></div>
									</c:if>
