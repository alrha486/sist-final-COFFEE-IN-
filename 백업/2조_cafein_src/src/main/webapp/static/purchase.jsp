...
$(document).ready(function(){ 
	...
	IMP.init('imp45627866'); // 'iamport' 대신 부여받은 "가맹점 식별코드"를 사용
...
/////////////////////페이지 로드될 때 회원 정보 알 수 있게하는 ajax	
	...
	var totalPrice =0; // 합계
    	for(var i=0; i<array.length; i++){ // 넘어온 상품 수만큼 돌면서 호출
    		$.ajax({  // 길이만큼 반복하면 ajax
    			url : "/coffee/productInfoSelect/" +array[i],
    			method : "GET",
    			success : function(data){			
    				var result = data.data;
    				str += "<tr>";
    				str += "<td>" + result[0].productname + "</td>"
    				str += "<td><img width='100px' src='static/img/product/"+result[0].productname+".jpg'></td>"
    				str += "<td>" + result[0].p_count + "</td>"
    				str += "<td>" + result[0].price1 + "</td>"
...
function payment(){ //결제버튼 눌렀을 때
	...
	if(checkval != 1 && checkval != 2 && checkval != 3 && checkval != 4){
		...
		IMP.request_pay({
		    ...
		    name : '주문명:결제테스트',              // 주문상품 이름
		    amount : 100,                    // 가격
		    buyer_email : $("#txtEmail").val(),   // 구매자 이메일
		    buyer_name : $("#username").val(),         // 구매자 이름
		    buyer_tel : $("#tel").val(),   // 구매자 전화번호
		    buyer_addr : $("#address2").val(),   // 구매자 주소
			...
			if ( rsp.success ) {  //  성공했을때...
			 	...
			 	$.ajax({ // 결제 
					...
					success : function(data){ // orderbasic의 orderid 가 넘어옴
						...
						for(var i=0; i<array.length; i++){ // 넘어온 상품 수만큼 돌면서 호출
							$.ajax({  // 길이만큼 반복하면 ajax
								url : "/coffee/orderDetailInsert/" +array[i] + "/"+ orderid, //회원가입 ajax
								method : "POST",
								success : function(data){	
						...
						$.ajax({  // 구매 완료 메일 보내는 ajax
								url : "/coffee/orderMail/" + orderid + "/" + totalPrice ,
								method : "POST",
								success : function(data){	
									if(data == 1){
									alert("주문이완료되었습니다.");
               ...
               <input type="text" name="address2" id="address2" required="required" placeholder="상세주소를 입력해주세요">
				...
				<select name='select' onchange="selectChange();">
				  ...
				  <option value=''>직접 입력</option>
					 ...
					 <input type="text" name="message" id="message" required="required" value ="" placeholder="배송시 참고할 사항이 있으면 입력해주세요" readonly="readonly">
            </div>
         </div>
      </div>
